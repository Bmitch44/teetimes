...
def scrape_glendenning_tee_times(html, course: Course, date: str) -> list[TeeTime]:
    start_time = t.time()
...
def fetch_glendenning_tee_times(date_str, course: Course):
    start_time = t.time()
    print(f"Fetching tee times for {course.label} on {date_str}")
    url = "https://www.tee-on.com/PubGolf/servlet/com.teeon.teesheet.servlets.golfersection.WebBookingAllTimesLanding?CourseCode=GGGG&Referrer="
...
if __name__ == "__main__":
    tee_times = fetch_glendenning_tee_times("2025-05-22", Course.GLADENNING)
    print(tee_times)
